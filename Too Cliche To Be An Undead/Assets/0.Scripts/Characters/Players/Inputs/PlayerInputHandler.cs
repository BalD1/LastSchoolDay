...
public class PlayerInputHandler : MonoBehaviourEventsHandler
{
    ...
    [field: SerializeField, ReadOnly] public Vector2 MovementsInput { get; private set; }

    public event Action<E_Devices> OnDeviceChange;
    public event Action<Vector2> OnMovementsInput;
    public event Action<Vector2> OnAimInput;
    public event Action OnAttackInput;
    public event Action OnDashInput;
    public event Action OnSkillInput;
    public event Action OnInteractionInput;
    public event Action OnSelfReviveInput;
    public event Action OnSecondContextualInput;
    ...
    private void CheckCurrentDevice()
    {
        ...
        this.OnDeviceChange?.Invoke(newType);
    }
    ...
    {
        MovementsInput = context.ReadValue<Vector2>();
        this.OnMovementsInput?.Invoke(MovementsInput);
    }

    public void OnAttack(InputAction.CallbackContext context)
    {
        if (context.performed) this.OnAttackInput?.Invoke();
    }

    public void OnDash(InputAction.CallbackContext context)
    {
        if (context.performed) this.OnDashInput?.Invoke();
    }

    public void OnSkill(InputAction.CallbackContext context)
    {
        if (context.performed) this.OnSkillInput?.Invoke();
    }

    public void OnInteract(InputAction.CallbackContext context)
    {
        if (context.performed) this.OnInteractionInput?.Invoke();
    }
    ...
    {
        if (context.performed) OnAimInput?.Invoke(context.ReadValue<Vector2>());
    }

    public void OnSelfRevive(InputAction.CallbackContext context)
    {
        if (context.performed) this.OnSelfReviveInput?.Invoke();
    }
    ...
    public void OnSecondContextual(InputAction.CallbackContext context)
    {
        ...
        {
            this.OnSecondContextualInput?.Invoke();
